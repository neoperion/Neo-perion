    ...
    <Link to={`/blog/${post.slug}`} className="group relative block rounded-3xl border border-white/[0.10] bg-gradient-to-br from-white/[0.04] to-white/[0.02] backdrop-blur-glass-1 overflow-hidden active:scale-[0.99] transition-transform">
      {post.coverImage && (
        <div className="relative aspect-[16/10] overflow-hidden">
          <img src={post.coverImage} alt="Blog cover" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between text-[11px] text-white/50"><span>AINCURU LLP</span><span>{post.readTime} min read</span></div>
      </div>
    </Link>
  );
}
        ...
        {categories.map((c) => (
          <button key={c} type="button" onClick={() => onChange(c)}
            className={cn('shrink-0 h-9 px-4 rounded-full text-[12px] font-semibold snap-center transition-all',
              active === c ? 'bg-gradient-to-br from-neo-blue to-neo-highlight text-white shadow-[0_4px_16px_rgba(247,126,13,0.3)]' : 'parchment-surface/[0.06] border border-white/[0.10] backdrop-blur-glass-1 text-white/75')}>{c}</button>
        ))}
      </div>
    </div>
  );
...
export function BlogReader({ title, author, publishedAt, readTime, coverImage, content, toc }: BlogReaderProps) {
  ...
  return (
    <article className="relative w-full pb-mobile-4xl">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button type="button" aria-label="Bookmark" onClick={() => setBookmarked((b) => !b)}
              className={cn('h-9 w-9 rounded-full flex items-center justify-center border transition-colors', bookmarked ? 'bg-neo-highlight/15 border-neo-highlight/30 text-neo-highlight' : 'parchment-surface/[0.05] border-white/[0.10] text-white/70')}>
              <Bookmark size={14} className={bookmarked ? 'fill-neo-highlight' : ''} />
            </button>
            <button type="button" aria-label="Share" className="h-9 w-9 rounded-full parchment-surface/[0.05] border border-white/[0.10] text-white/70 flex items-center justify-center"><Share2 size={14} /></button>
            {toc && toc.length > 0 && (
              <button type="button" aria-label="Table of contents" onClick={() => setTocOpen(true)} className="h-9 w-9 rounded-full parchment-surface/[0.05] border border-white/[0.10] text-white/70 flex items-center justify-center"><List size={14} /></button>
            )}
          </div>
        </div>
      ...
      {coverImage && <div className="relative aspect-[16/9] mx-mobile-base rounded-3xl overflow-hidden border border-white/[0.10] mb-7"><img src={coverImage} alt={title} className="w-full h-full object-cover" /></div>}
      ...
      {tocOpen && toc && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-mobile-overlay parchment-surface--deep/60 backdrop-blur-glass-2" onClick={() => setTocOpen(false)}>
          ...
          >
            <div className="flex justify-center mb-4"><span className="h-1 w-10 rounded-full parchment-surface/30" /></div>
            <div className="flex items-center justify-between mb-4"><h2 className="text-sm font-bold text-white uppercase tracking-[0.18em]">Contents</h2>
              <button onClick={() => setTocOpen(false)} className="h-8 w-8 rounded-full parchment-surface/[0.06] text-white/70 flex items-center justify-center"><X size={14} /></button>
            </div>
            <ul className="space-y-1">{toc.map((t) => (<li key={t.id}><a href={`#${t.id}`} onClick={() => setTocOpen(false)} className="block px-3 py-3 rounded-xl text-[14px] text-white/80 hover:parchment-surface/[0.06] transition-colors">{t.label}</a></li>))}</ul>
          </motion.aside>
        </motion.div>
      )}
    </article>
