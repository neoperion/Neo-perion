...
export function ConversationForm({ intro = "Hi! Let's get to know you.", fields, ctaLabel = 'Send', onSubmit }: ConversationFormProps) {
  ...
  return (
    <div className="relative w-full px-mobile-base py-mobile-3xl">
      ...
      <div className="rounded-3xl border border-white/[0.10] parchment-surface/[0.03] backdrop-blur-glass-1 p-5 overflow-hidden">
          ...
          {done ? (
            ...
            <motion.div key={current.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
              ...
              {current.options ? (
                <div className="flex flex-wrap gap-2">{current.options.map((o) => (
                  <button key={o} type="button" onClick={() => { setValue(o); setTimeout(next, 250); }}
                    className={cn('h-11 px-4 rounded-full text-[13px] font-semibold transition-all', value === o ? 'bg-gradient-to-br from-neo-blue to-neo-highlight text-white' : 'parchment-surface/[0.05] border border-white/[0.10] text-white/85')}>{o}</button>
                ))}</div>
              ...
              <div className="mt-5 flex items-center gap-2">
                {step > 0 && <button type="button" onClick={() => setStep((s) => Math.max(0, s - 1))} className="h-12 px-4 rounded-2xl parchment-surface/[0.05] border border-white/[0.10] text-white font-semibold text-sm">Back</button>}
      ...
      <div className="mt-5 flex gap-1.5">{fields.map((f, i) => (<div key={f.id} className={cn('h-1 flex-1 rounded-full', i <= step ? 'bg-gradient-to-r from-neo-blue to-neo-highlight' : 'parchment-surface/[0.10]')} />))}</div>
    </div>
  );
}
