...
export function ServiceCarouselMobile({ eyebrow = 'Services', title = 'Engineering Capabilities', subtitle = 'Explore our expertise.', services }: ServiceCarouselMobileProps) {
  ...
  return (
    <section ref={containerRef} className="relative w-full py-16 bg-neutral-900" aria-labelledby="svc-heading">
      <div className="px-mobile-base mb-8">
        <p className="text-[11px] font-bold tracking-[0.25em] uppercase text-neo-blue mb-2">{eyebrow}</p>
        <h2 id="svc-heading" className="text-[clamp(28px,7vw,36px)] font-bold text-white tracking-tight font-display">{title}</h2>
      ...
      <div className="relative px-mobile-base pb-10 flex flex-col gap-6">
        {services.map((s, i) => {
          ...
          return (
            ...
            >
              <Link to={s.href} className="block w-full rounded-[28px] overflow-hidden bg-neutral-900 shadow-[0_8px_40px_rgba(0,0,0,0.06)] border border-neutral-800 p-8 transform transition-transform hover:-translate-y-1">
                <div className="relative flex flex-col gap-6">
                  <div className="h-14 w-14 rounded-2xl bg-neutral-900 border border-neutral-800 flex items-center justify-center text-neo-blue shadow-sm">
                    <Icon size={28} />
                  </div>
                  <div>
                    <h3 className="text-2xl font-bold text-white tracking-tight font-display mb-2">{s.title}</h3>
                    <p className="text-[15px] text-neutral-400 leading-relaxed font-medium">{s.description}</p>
                  </div>
