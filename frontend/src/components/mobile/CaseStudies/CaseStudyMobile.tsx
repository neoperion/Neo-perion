    ...
    <Link to={`/company/case-studies/${study.slug}`} className="group relative block rounded-3xl border border-white/[0.10] bg-gradient-to-br from-white/[0.04] to-white/[0.02] backdrop-blur-glass-1 overflow-hidden active:scale-[0.99] transition-transform">
      {study.coverImage && (
        <div className="relative aspect-[16/10] overflow-hidden">
          <img src={study.coverImage} alt="Case study cover" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
      ...
      <div className="p-4">
        ...
        {study.techStack.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {study.techStack.slice(0, 3).map((t) => (<span key={t} className="h-6 px-2 rounded-full parchment-surface/[0.05] border border-white/[0.10] text-[10px] font-medium text-white/75">{t}</span>))}
...
export function CaseStudyStory({ eyebrow, client, title, outcome, problem, challenge, solution, results, metrics, heroImage, techStack }: CaseStudyStoryProps) {
  ...
  return (
    <article className="relative w-full pb-mobile-4xl">
      ...
      {heroImage && <div className="relative aspect-[16/9] mx-mobile-base rounded-3xl overflow-hidden border border-white/[0.10] mb-7"><img src={heroImage} alt="Case study cover" className="w-full h-full object-cover" /></div>}
          ...
          <div className="grid grid-cols-3 gap-2">
            {metrics.map((m, i) => (
              <motion.div key={m.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, type: 'spring', stiffness: 320, damping: 32 }}
                className="relative rounded-2xl border border-white/[0.10] parchment-surface/[0.04] backdrop-blur-glass-1 p-3 text-center overflow-hidden">
      ...
      <div className="px-mobile-base mb-5">
        <div className="flex gap-2 p-1 rounded-full parchment-surface/[0.04] border border-white/[0.08] backdrop-blur-glass-1">
          {sections.map((s, i) => (
      ...
      <div className="px-mobile-base">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div key={sections[section].id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}
            className="rounded-3xl border border-white/[0.10] parchment-surface/[0.04] backdrop-blur-glass-1 p-5">
      ...
      {techStack && techStack.length > 0 && (
        <section className="px-mobile-base mt-7">
          ...
          <div className="flex flex-wrap gap-2">{techStack.map((t) => (<span key={t} className="h-8 px-3 rounded-full parchment-surface/[0.05] border border-white/[0.10] text-[11px] font-medium text-white/85 flex items-center">{t}</span>))}</div>
        </section>
      )}
      <div className="px-mobile-base mt-9"><Link to="/company/case-studies" className="w-full h-12 rounded-2xl parchment-surface/[0.06] border border-white/[0.12] backdrop-blur-glass-1 text-white font-semibold text-sm flex items-center justify-center gap-2 active:scale-[0.98]">View All Case Studies <ChevronRight size={14} /></Link></div>
    </article>
  );
}
