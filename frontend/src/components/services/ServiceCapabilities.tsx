      ...
      <div className="container mx-auto max-w-[1200px] px-6 lg:px-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {service.features.map((feature) => (
            <div
              key={feature.title}
              className="group rounded-xl border border-manuscriptAlpha-ink-15 bg-manuscript-parchmentLight p-8 transition-colors hover:border-brand/30"
            >
              <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl border border-manuscriptAlpha-ink-15 bg-manuscript-parchmentDark">
                <Icon className="text-brand" size={22} />
              </div>
              <h3 className="mb-3 text-xl font-bold text-manuscript-ink">{feature.title}</h3>
              <p className="leading-relaxed text-manuscript-inkSoft">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
