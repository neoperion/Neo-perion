...
export function CareersHero({ theme = 'dark' }: CareersHeroProps) {
  ...
  return (
    <section className={`relative min-h-[60vh] flex items-center pt-36 pb-20 overflow-hidden ${isLight ? 'bg-[#FAFAFA]' : 'bg-[#02040A]'}`}>
      ...
      <div className="max-w-4xl mx-auto px-8 relative z-10 text-center">
        ...
        <p className={`text-lg leading-relaxed max-w-2xl mx-auto mb-10 animate-fade-in-up ${isLight ? 'text-slate-500' : 'text-slate-400'}`} style={{animationDelay: '200ms'}}>
        ...
        <div className="animate-fade-in-up" style={{animationDelay: '300ms'}}>
          <button
            ...
            className={`px-8 py-3.5 rounded-lg text-sm font-bold transition-all duration-300 ${
              isLight 
                ? 'bg-neo-blue text-white hover:bg-blue-600 shadow-sm'
                : 'bg-neo-blue text-white hover:bg-blue-600'
            }`}
            style={isLight ? {} : { boxShadow: '0 8px 30px -6px rgba(37,99,255,0.4)' }}
          >
            View Open Roles
          </button>
        </div>
